...
class Conditional_StyledGenerator(nn.Module):
    def __init__(self, class_num):
        super().__init__()

        self.embed = nn.Sequential(
            nn.Embedding(num_embeddings=class_num,embedding_dim=512),
            nn.ReLU(),
            nn.Linear(512,512),
            nn.ReLU(),
            nn.Linear(512, 512)
            )
        self.style_generator = StyledGenerator()

    def forward(self, input, class_id):

        class_embed = self.embed(class_id)
        input = input*class_embed
        out = self.style_generator(input)
        return out
...
class StyledGenerator(nn.Module):
    ...
    def forward(self, input, noise=None, step=4, alpha=1, mean_style=None, style_weight=0, mixing_range=(-1, -1)):
        styles = []
...
class Discriminator(nn.Module):
    ...
    def forward(self, input, step=4, alpha=1):
